...
Easy-Novels is built for multiple operating systems and architectures. Every release includes binaries for all supported platforms.
...
- Install with: `sudo dpkg -i Easy-Novels-*.deb` or double-click

### RPM

- Package manager format for Red Hat-based distributions
- Fedora, RHEL, CentOS, openSUSE, etc.
- Install with: `sudo rpm -i Easy-Novels-*.rpm` or double-click
...
1. Electron (the framework Easy-Novels is built on) does not officially support RISC-V yet
