...
let mainWindow;
const store = new Store({ name: "easy-novels-preferences" });
        ...
        { type: "separator" },
        {
          ...
          click: () => {
            dialog.showMessageBox(mainWindow, {
              ...
              message: "Novelist",
              detail:
                "A distraction-free writing app for easy-novelss.\\nVersion 1.0.0",
            });
          },
        },
      ],
...
ipcMain.handle(
  "chapters:save",
  async (_event, projectPath, chapterId, content) => {
    await ensureProjectStructure(projectPath);
...
ipcMain.handle(
  "chapters:create-scene",
  async (_event, projectPath, chapterId, sceneName) => {
    await ensureProjectStructure(projectPath);
...
ipcMain.handle(
  "chapters:save-scene",
  async (_event, projectPath, chapterId, sceneId, content) => {
    await ensureProjectStructure(projectPath);
...
ipcMain.handle(
  "chapters:delete-scene",
  async (_event, projectPath, chapterId, sceneId) => {
    await ensureProjectStructure(projectPath);
...
ipcMain.handle(
  "scenes:reorder",
  async (_event, projectPath, chapterId, sceneIds) => {
    await ensureProjectStructure(projectPath);
...
ipcMain.handle(
  "characters:save",
  async (_event, projectPath, characterId, content) => {
    await ensureProjectStructure(projectPath);
...
ipcMain.handle(
  "characters:delete",
  async (_event, projectPath, characterId) => {
    await ensureProjectStructure(projectPath);
...
ipcMain.handle(
  "git:configure-user",
  async (_event, projectPath, username, email) => {
